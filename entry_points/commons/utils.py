...
import inference.nn_model_training as nn_model
...
def split_column_type(features: Dict[str, str], exclude_cols: Iterable[str] = None):
    ...
    categorical_cols = [
        ...
        if v == "STRING" and k.lower() not in exclude_cols
    ]
    numerical_cols = [
        k
        for k, v in features.items()
...
def query_data(
    ...
    if predict_data:
        query = (
            query + f" AND MOD(ABS(FARM_FINGERPRINT({id_column})), {splits}) = {split}"
        )
...
def preprocess(
    data, num_cols, cat_cols, transformer, label=None, train=True, dense=False
):
    if type(label) == list:
        if len(label) > 1:
            pass
            ...
            label = label[0]
    if train:
        transformer.fit(data[[*num_cols, *cat_cols]])

    if not dense:
        X = coo_matrix(
            transformer.transform(data[[*num_cols, *cat_cols]]), dtype=np.float16
        ).tocsc()
    else:
        X = transformer.transform(data[[*num_cols, *cat_cols]])

    if label is not None:
        y = data[label].reset_index(drop=True).astype("float32")
        return X, y
...
class Multi_Class_Objective(object):
    ...
    def __call__(self, trial):
        ...
        param = {
            ...
            "min_gain_to_split": trial.suggest_discrete_uniform(
                "min_gain_to_split", 0.1, 5, 0.01
            ),
        ...
        sample = np.random.choice(
            self.X.shape[0], size=int(self.sample_size), replace=False
        )
...
def load_tf_model(config_path: str, weight_path: str, model_name: str, bucket: str):
    custom_objects = {
        "TokenAndPositionEmbedding": nn_model.TokenAndPositionEmbedding,
        "TransformerBlock": nn_model.TransformerBlock,
    }
...
def query_to_table(
    ...
    if destination_partition_field:
        job_config.time_partitioning = bigquery.TimePartitioning(
            type_=bigquery.TimePartitioningType.DAY, field=destination_partition_field
        )
    ...
    job.result()
    print(
        f"Job done for'{destination_table}'. Job ID: {job.job_id}. "
    ...
    if train:
        ...
        for name in attributes:
            ...
            tokenized_text = tokenizer.texts_to_sequences(
                data.loc[data["DATA_LABEL"] == "TRAIN"][name]
            )
            tokenizers[name] = {}
            ...
            tokenized_data[name][
                ...
                tokenized_text, maxlen=tokenizers[name]["maxlen"], padding="post"
            )
            for label in ["TEST", "EVAL"]:
                tokenized_text = tokenizer.texts_to_sequences(
                    data.loc[data["DATA_LABEL"] == label][name]
                )
                tokenized_data[name][
                    label
                ] = tf.keras.preprocessing.sequence.pad_sequences(
        ...
        for name in attributes:
            ...
            tokenized_data[name] = tf.keras.preprocessing.sequence.pad_sequences(
                tokenized_text, maxlen=tokenizer[name]["maxlen"], padding="post"
            )
